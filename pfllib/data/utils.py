...
def separate_data(
    ...
    if partition == "pat":
        ...
        K = num_classes
        N = len(y_train)
        logger.debug("clientidx_map: %s", clientidx_map)
        logger.debug(
            "Number of clients per label: %s",
            [len(clientidx_map[i]) for i in range(len(clientidx_map))],
        )
    ...
    for client in range(num_clients):
        logger.info(f"Client {client}\t Size of data: {len(X[client])}\t Labels: {np.unique(y[client])}")
        logger.debug(f"\t\t Samples of labels: {[i for i in statistic[client]]}")
...
def split_data(X, y, train_ratio=DEFAULT_TRAIN_RATIO):
    ...
    for i in range(len(y)):
        X_train, X_test, y_train, y_test = train_test_split(X[i], y[i], train_size=train_ratio, shuffle=True)
    ...
    logger.debug("The number of train samples: %s", num_samples["train"])
    logger.debug("The number of test samples: %s", num_samples["test"])
    del X, y
