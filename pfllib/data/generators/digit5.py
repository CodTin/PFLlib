...
def generate_dataset(dir_path):
    ...
    if not os.path.exists(root):
        ...
        os.system(
            f"wget https://drive.google.com/u/0/uc?id=1PT6K-_wmsUEUCxoYzDy0mxF-15tvb2Eu&export=download -P {root}"
        )
    ...
    num_clients = len(y)
    logger.info(f"Number of labels: {labelss}")
    ...
    for client in range(num_clients):
        ...
        logger.info(f"\t\t Samples of labels: {[i for i in statistic[client]]}")
        logger.info("-" * 50)
