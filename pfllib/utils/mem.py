...
class MemReporter:
    ...
    def __init__(self, model: Optional[torch.nn.Module] = None):
        self.tensor_name: dict[int, str] = {}
        self.device_mapping: defaultdict[torch.device, list[torch.Tensor]] = defaultdict(list)
        self.device_tensor_stat: dict[torch.device, list[tuple[str, tuple[int, ...], int, int]]] = {}
        ...
        for param, names in tensor_names.items():
            self.tensor_name[id(param)] = "+".join(names)
    ...
    def get_stats(self):
        ...
        def get_tensor_stat(self, tensor: torch.Tensor) -> List[Tuple[str, Tuple[int, ...], int, int]]:
            ...
            if tensor.is_sparse:
                indices_stat = self.get_tensor_stat(tensor._indices())
                values_stat = self.get_tensor_stat(tensor._values())
            ...
            for tensor in tensors:
                ...
                if isinstance(tensor, torch.nn.Parameter):
                    if tensor.grad is not None:
                        ...
                        self.tensor_name[id(tensor.grad)] = "{}.grad".format(self._get_tensor_name(tensor))
        ...
        for device, tensor_stats in self.device_tensor_stat.items():
            ...
            if device != torch.device("cpu"):
                ...
                if memory_allocated != total_mem:
                    print("Memory differs due to the matrix alignment or invisible gradient buffer tensors")
            print("-" * LEN)
