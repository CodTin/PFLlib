...
class ALA:
    def __init__(
        ...
        self.weights: Optional[List[torch.Tensor]] = None  # Learnable local aggregation weights.
    ...
    def adaptive_local_aggregation(self, global_model: nn.Module, local_model: nn.Module) -> None:
        ...
        rand_loader: DataLoader = DataLoader(self.train_data[rand_idx : rand_idx + rand_num], self.batch_size, drop_last=False)  # type: ignore[arg-type]
        ...
        while True:
            for x, y in rand_loader:
                ...
                for param_t, param, param_g, weight in zip(params_tp, params_p, params_gp, self.weights):  # type: ignore[arg-type]
