...
class FedPAC(Server):
    ...
    def evaluate(self, acc=None, loss=None):
        ...
        logger.info("Averaged Test Accuracy: {:.4f}".format(test_acc))
    ...
    def aggregate_and_send_heads(self):
        ...
        for idx, cid in enumerate(self.uploaded_ids):
            logger.info("(Client {}) Weights of Classifier Head".format(cid))
            logger.info(head_weights[idx], "\n")
    ...
    for i in range(num_users):
        ...
        for ii in range(num_users):
            if evals[ii].real >= 0.01:
                p_matrix_new += evals[ii].real * torch.mm(
                    evecs[:, ii].reshape(num_users, 1), evecs[:, ii].reshape(1, num_users)
                )
