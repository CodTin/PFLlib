...
class ClientCAC(Client):
    ...
    def evaluate_critical_parameter(self, prevModel: nn.Module, model: nn.Module, tau: float):
        ...
        """
        global_mask: list[torch.Tensor] = []  # mark non-critical parameter
        local_mask: list[torch.Tensor] = []  # mark critical parameter
        critical_parameter_parts: list[torch.Tensor] = []
        ...
        for (name1, prevparam), (name2, param) in zip(prevModel.named_parameters(), model.named_parameters()):
            ...
            if thresh <= 1e-10:
                ...
                if len(new_metric) == 0:  # this means all items in metric are zero
                    logger.warning(f"Abnormal!!! metric:{metric}")
                else:
            ...
            critical_parameter_parts.append(mask.view(-1))
        model.zero_grad()
        critical_parameter = torch.cat(critical_parameter_parts)
    ...
    def set_parameters(self, model):
        if self.local_mask is not None:
            ...
            for (name1, param1), (name2, param2), (name3, param3) in zip(
                self.model.named_parameters(), model.named_parameters(), self.customized_model.named_parameters()
            ):
                param1.data = (
