...
class ClientAS(Client):
    ...
    def set_parameters(self, model, progress):
        ...
        alignment_optimizer = torch.optim.SGD(
            model.base.parameters(), lr=0.01
        )  # Adjust learning rate and optimizer as needed
            ...
            for x_batch, y_batch in trainloader:
                ...
                for label in y_batch.unique():
                    if mean_prototypes[label.item()] is not None:
                        ...
                        class_loss = alignment_loss_fn(
                            global_proto_batch[y_batch == label], mean_prototypes[label.item()]
                        )
