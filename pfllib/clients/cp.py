...
class ClientCP(Client):
    ...
    def train_cs_model(self):
        ...
        logger.info(f"scores mean={np.mean(scores):.4f}, std={np.std(scores):.4f}")
...
class Gate(nn.Module):
    def __init__(self, cs) -> None:
        ...
        self.cs = cs
        self.pm: list[torch.Tensor] = []
        self.gm: list[torch.Tensor] = []
        self.pm_: list[torch.Tensor] = []
        self.gm_: list[torch.Tensor] = []
